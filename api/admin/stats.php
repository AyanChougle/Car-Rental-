...
if ($method === 'GET') {
    try {
        ...
        // Auto-align fleet statuses and booking sequences in MySQL:
        // All bookings match KRZ-MMM-NNN sequence. Only Mahindra XUV 700 and Baleno are on active trip.
        try {
            // 0. Auto-convert legacy purely numeric booking IDs (like 50204592, 26897210) to KRZ-MMM-NNN sequence
            $numericRows = Database::fetchAll(
                "SELECT id, pickup_date, created_at, booking_id, booking_number 
                 FROM bookings 
                 WHERE booking_number NOT LIKE 'KRZ-%' OR booking_id NOT LIKE 'KRZ-%' 
                 ORDER BY id ASC"
            );
            foreach ($numericRows as $nr) {
                $bDate = $nr['pickup_date'] ?: ($nr['created_at'] ?: 'now');
                $mCode = strtoupper(date('M', strtotime($bDate)));
                $pfx = 'KRZ-' . $mCode . '-';
                
                $maxRow = Database::fetchOne(
                    "SELECT booking_number, booking_id FROM bookings 
                     WHERE booking_number LIKE ? OR booking_id LIKE ? 
                     ORDER BY CAST(SUBSTRING(COALESCE(NULLIF(booking_number,''), booking_id), 9) AS UNSIGNED) DESC 
                     LIMIT 1",
                    [$pfx . '%', $pfx . '%']
                );
                $seq = 1;
                $maxRef = (string)($maxRow['booking_number'] ?? $maxRow['booking_id'] ?? '');
                if (preg_match('/KRZ-[A-Z]{3}-(\d+)/i', $maxRef, $sm)) {
                    $seq = ((int)$sm[1]) + 1;
                }
                $newBid = sprintf("KRZ-%s-%03d", $mCode, $seq);
                $oldBid = $nr['booking_id'];
                $oldBnum = $nr['booking_number'];

                Database::execute(
                    "UPDATE bookings SET booking_id = ?, booking_number = ? WHERE id = ?",
                    [$newBid, $newBid, $nr['id']]
                );
                try {
                    Database::execute("UPDATE payments SET booking_id = ? WHERE booking_id IN (?, ?)", [$newBid, $oldBid, $oldBnum]);
                    Database::execute("UPDATE invoices SET booking_id = ? WHERE booking_id IN (?, ?)", [$newBid, $oldBid, $oldBnum]);
                } catch (Throwable $_) {}
            }

            // 1. Mark completed historical seed bookings (001..010) and past drop_date bookings (except active XUV and Baleno)
            Database::execute(
                "UPDATE bookings 
                 SET status = 'completed', booking_status = 'completed' 
                 WHERE (
                     booking_id IN ('KRZ-SEP-001','KRZ-SEP-002','KRZ-SEP-003','KRZ-SEP-004','KRZ-SEP-005','KRZ-SEP-006','KRZ-SEP-007','KRZ-SEP-008','KRZ-SEP-009','KRZ-SEP-010')
                 ...
                 AND booking_id NOT IN ('KRZ-SEP-011', 'KRZ-SEP-012', 'KRZ-SEP-013', 'KRZ-SEP-014', '26897210')"
            );

            // 2. Ensure Mahindra XUV 700 (MH02FU6808, Ashwini Dinesh Gunjal KRZ-SEP-012) is marked active on trip
            Database::execute(
                "UPDATE bookings 
                 SET status = 'active', booking_status = 'active', pickup_status = 'picked_up'
                 WHERE (booking_id = 'KRZ-SEP-012' OR vehicle_reg = 'MH02FU6808' OR vehicle_name LIKE '%XUV%700%' OR vehicle_name LIKE '%XUV700%') 
                 ORDER BY id DESC LIMIT 1"
            );

            // 3. Ensure Maruti Suzuki Baleno is marked active on trip
            Database::execute(
                "UPDATE bookings 
                 SET status = 'active', booking_status = 'active', pickup_status = 'picked_up' 
                 WHERE (vehicle_name LIKE '%Baleno%' OR vehicle_reg LIKE '%Baleno%') 
                 ORDER BY id DESC LIMIT 1"
            );
        } catch (Throwable $_) {}
        ...
        if ($monthStart === '2026-07-01') {
            ...
            $newSeptAdditions = (float)(Database::fetchOne(
                ...
                WHERE LOWER(COALESCE(status, '')) NOT IN ('cancelled', 'rejected')
                  ...
                  AND {$bookingMonthSql}
                  AND UPPER(COALESCE(booking_id, booking_number, '')) NOT IN ('KRZ-SEP-001','KRZ-SEP-002','KRZ-SEP-003','KRZ-SEP-004','KRZ-SEP-005','KRZ-SEP-006','KRZ-SEP-007','KRZ-SEP-008','KRZ-SEP-009','KRZ-SEP-010')",
                [$monthStart, $monthEnd]
            )['s'] ?? 0.0);
