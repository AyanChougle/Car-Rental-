...
function bookingCard(b, isLive) {
  ...
  if (isCancelledOrRejected) {
    waMsgContent =
      ...
      `Reason: ${b.cancellationReason || b.cancellation_reason || b.paymentRejectionReason || "Cancelled"}\n\n` +
      `Kindly assist me regarding this cancellation.`;
  } else {
    waMsgContent =
  ...
  const waText = encodeURIComponent(waMsgContent);
  const waUrl = `https://wa.me/919167164547?text=${waText}`;

  const waBtnColor = isCancelledOrRejected ? "#ef476f" : "#25d366";
  const waBtnLabel = isCancelledOrRejected ? "WhatsApp Cancellation" : "WhatsApp Confirmation";

  const whatsappBtnHtml = `
    <a href="${waUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-outline" style="border-color:${waBtnColor};color:${waBtnColor};display:inline-flex;align-items:center;gap:6px;font-size:0.85rem;padding:8px 14px;">
